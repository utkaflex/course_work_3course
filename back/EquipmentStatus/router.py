...
router = APIRouter(prefix="/equipment_status", tags=["Статусы оборудования"])
...
async def update_equipment_status(
    ...
    if not existing_status:
        raise HTTPException(status_code=404, detail="Equipment status not found")
    return await crud.update_equipment_status(
        status_id=status_id, updated_status=updated_status
    )
