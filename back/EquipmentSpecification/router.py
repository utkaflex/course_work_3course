...
from EquipmentSpecification.schemas import (
    SEquipmentSpecification,
    SEquipmentSpecificationCreate,
)

router = APIRouter(prefix="/equipment_specs", tags=["Технические характеристики"])
...
async def update_equipment_specification(
    spec_id: int, updated_spec: SEquipmentSpecificationCreate
):
    existing_spec = await crud.get_equipment_specifications(spec_id)
    if not existing_spec:
        raise HTTPException(status_code=404, detail="Equipment specification not found")
    return await crud.update_equipment_specifications(
        spec_id=spec_id, updated_spec=updated_spec
    )
