...
router = APIRouter(prefix="/equipment_status_type", tags=["Типы статусов оборудования"])
...
async def create_equipment_status_type(status_type: SEquipmentStatusTypeCreate):
    db_status_type = await crud.get_equipment_status_type_by_name(
        status_type_name=status_type.status_type_name
    )
    if db_status_type:
        raise HTTPException(
            status_code=400, detail="Equipment status type already exists"
        )
...
async def update_equipment_status_type(
    status_type_id: int, updated_status_type: SEquipmentStatusTypeCreate
):
    ...
    db_status_type = await crud.get_equipment_status_type_by_name(
        status_type_name=updated_status_type.status_type_name
    )
    if db_status_type and db_status_type.id != status_type_id:
        raise HTTPException(
            status_code=400,
            detail="Equipment status type name already in use by another status type",
        )

    return await crud.update_equipment_status_type(
        status_type_id=status_type_id, updated_status_type=updated_status_type
    )
