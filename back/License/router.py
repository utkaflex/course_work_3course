...
router = APIRouter(prefix="/license", tags=["Работа с лицензиями"])
...
async def update_License(license_id: int, updated_license: SLicenseCreate):
    ...
    db_license = await crud.get_license_by_type(
        license_type=updated_license.license_type
    )
    if db_license and db_license.id != license_id:
        raise HTTPException(
            status_code=400, detail="License name already in use by another License"
        )

    return await crud.update_license_type(
        license_id=license_id, new_license_type=updated_license.license_type
    )
