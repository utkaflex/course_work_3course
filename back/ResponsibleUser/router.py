...
from ResponsibleUser.schemas import (
    SAllResponsibleUser,
    SResponsibleUser,
    SResponsibleUserCreate,
)
...
router = APIRouter(prefix="/responsible_users", tags=["Работа с ответственными"])
...
async def update_responsible_user(user_id: int, updated_user: SResponsibleUserCreate):
    ...
    return await crud.update_responsible_user(
...
async def update_job(job_id: int, updated_job: SResponsibleUserJobCreate):
    ...
    db_job = await crud_responsible_user_job.get_job_by_name(
        job_name=updated_job.job_name
    )
    if db_job and db_job.id != job_id:
        raise HTTPException(
            status_code=400, detail="Job name already in use by another job"
        )

    return await crud_responsible_user_job.update_job(
        job_id=job_id, new_job_name=updated_job.job_name
    )
...
async def create_office(office: SResponsibleUserOfficeCreate):
    db_office = await crud_responsible_user_office.get_office_by_name(
...
async def update_office(office_id: int, updated_office: SResponsibleUserOfficeCreate):
    ...
    db_office = await crud_responsible_user_office.get_office_by_name(
        office_name=updated_office.office_name
    )
    if db_office and db_office.id != office_id:
        raise HTTPException(
            status_code=400, detail="Office name already in use by another office"
        )

    return await crud_responsible_user_office.update_office(
        office_id=office_id, new_office_name=updated_office.office_name
    )
