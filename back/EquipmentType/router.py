...
router = APIRouter(prefix="/equipment_types", tags=["Типы оборудования"])


@router.post("/create", response_model=SEquipmentType)
async def create_equipment_type(equipment_type: SEquipmentTypeCreate):
    db_equipment_type = await crud.get_equipment_type_by_name(
...
async def update_equipment_type(
    type_id: int, updated_equipment_type: SEquipmentTypeCreate
):
    ...
    db_equipment_type = await crud.get_equipment_type_by_name(
        type_name=updated_equipment_type.type_name
    )
    if db_equipment_type and db_equipment_type.id != type_id:
        raise HTTPException(
            status_code=400, detail="Equipment type name already in use by another type"
        )

    return await crud.update_equipment_type(
        type_id=type_id, new_type_name=updated_equipment_type.type_name
    )
