...
async def login_for_access_token(
    ...
    refresh_token = auth.create_refresh_token(
        data={"sub": db_user.username},
        expires_delta=timedelta(days=refresh_token_expires),
    )
    ...
    await session_log_event(
        session_log=SSessionLogCreate(event_type="login", user_agent=""), user=db_user
    )
...
async def refresh_access_token(response: Response, refresh_token: str):
    ...
    await session_log_event(
        session_log=SSessionLogCreate(event_type="refresh", user_agent=""), user=db_user
    )
