...
async def generate_equipment_excel(
    payload: SEquipmentExcelReport, user: User = Depends(get_current_user)
):
    ...
    try:
        equipment_period = (
            await crud.get_equipment_for_ready_report(payload.ids)
            if hasattr(crud, "get_equipment_for_ready_report")
            else await crud.get_equipment_by_ids(payload.ids)
        )
...
async def create_equipment(equipment: SEquipmentCreate):
    db_equipment = await crud.get_equipment_by_serial_number(
        serial_number=equipment.serial_number
    )
    if db_equipment:
        raise HTTPException(
            status_code=400, detail="Equipment with this serial number already exists"
        )
...
async def check_inventory_number(inventory_number: str):
    ...
    if equipment:
        raise HTTPException(
            status_code=400,
            detail="Equipment with this inventory number already exists",
        )
...
) -> List[SEquipmentWithResponsible]:
...
async def update_equipment(equipment_id: int, updated_equipment: SEquipmentCreate):
    ...
    return await crud.update_equipment(
        equipment_id=equipment_id, updated_equipment=updated_equipment
    )
...
async def generate_equipment_word(
    equipment_id: int, user: User = Depends(get_current_user)
):
    ...
    fields = [
        ...
        (
            "Кому выдано:",
            (
                equipment.responsible_user_full_name
                if equipment.responsible_user_full_name
                else "Не указано"
            ),
        ),
