    ...
    async with async_session() as session:
        async with session.begin():
            ...
            if software.contract_ids:
                software_contracts = [
                    SoftwareContract(
                        software_id=db_software.id, contract_id=contract_id
                    )
...
async def get_software_for_excel(software_list: List[SSoftwareAll]):
    async with async_session() as session:
        ...
        query = (
            select(Software)
            .where(Software.id.in_(software_ids))
            .options(
                joinedload(Software.contracts).joinedload(SoftwareContract.contract),
                joinedload(Software.license),
            )
...
async def add_contract_to_software(software_id: int, contract_id: int):
    async with async_session() as session:
        ...
        existing_link = await session.execute(
            select(SoftwareContract).filter_by(
                software_id=software_id, contract_id=contract_id
            )
        )
        if existing_link.scalar():
            raise HTTPException(
                status_code=400,
                detail="This contract is already linked to the software",
            )
