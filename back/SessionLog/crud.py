...
async def session_log_event(
    session_log: SSessionLogCreate, user: User = Depends(get_current_user)
):
    async with async_session() as session:
        db_session_log = SessionLog(
            event_type=session_log.event_type,
            user_id=user.id,
...
async def delete_old_session_logs(max_logs: int = 100):
    async with async_session() as session:
        subquery = (
            select(SessionLog.id).order_by(SessionLog.time.desc()).limit(max_logs)
        )
