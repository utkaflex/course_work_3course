...
router = APIRouter(prefix="/building", tags=["Работа с корпусами"])


@router.post("/create", response_model=SBuilding)
async def create_building(building: SBuildingCreate):
    db_building = await crud.get_building_by_address(
...
async def update_building(building_id: int, updated_building: SBuildingCreate):
    ...
    db_building = await crud.get_building_by_address(
        building_address=updated_building.building_address
    )
    if db_building and db_building.id != building_id:
        raise HTTPException(
            status_code=400,
            detail="Building address already in use by another building",
        )

    return await crud.update_building_address(
        building_id=building_id, new_building_address=updated_building.building_address
    )
